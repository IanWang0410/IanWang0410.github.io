...
if(isset($_POST['id'])){
    ...
    $stmt = $conn->prepare("UPDATE customer SET `CUSTOMER_Name`=?, `CUSTOMER_Email`=?, `CUSTOMER_Address`=?, `CUSTOMER_HouseTelpNo`=?, `CUSTOMER_PhoneNo`=? WHERE `CUSTOMER_Id`=?");
    $stmt->bind_param("sssssi", $name, $email, $address, $houseTel, $phone, $id);
        if($stmt->execute()) {
            echo "success";
        } else {
